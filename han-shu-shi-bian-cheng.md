---
description: >-
  Java是一门面向对象的语言，虽然Java8中引入了lambda表达式，或我们可以在Android
...
虽然内联函数会帮助我们提升性能，但我们可能不应该频繁的使用它们，因为：
...
有些时候我们想在嵌套函数中使用内联，这时候编译器会提示我们错误，因为内联函数允许非本地返回，而嵌套函数有可能在其他的上下文中执行时，这种操作不应该被允许。解决的办法是使用**跨内联修饰符**（crossinline modifier），通知编译器不允许非本地返回：
