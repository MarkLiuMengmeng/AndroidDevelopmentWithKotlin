...
如果子类没有构造器而超类有非空的构造器，则每个次构造器都要使用`super`关键字来初始化基类：
...
如果想使用超类的方法可以使用`super`关键字加方括号引用类名：
...
我们已经知道所有类型的超类型是`Any`，等同于Java中的`Object`。每个类都隐式或显式地继承`Any`：
...
* 抽象类中可以有抽象成员也可以有普通成员
* 如果一个子类实现了超类（抽象类）的所有抽象方法，那么该子类可以不必是抽象类，否则就依然是抽象类
